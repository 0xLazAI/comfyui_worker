...
const EXPORT_EPILOGUE_BODY = [
  ...
  // export_cameras/export_animations default to False/True in Blender's glTF
  // exporter; cameras must be opt-in or every PACE camera (and the per-shot
  // 运镜 keyframes that live on those camera objects) is silently dropped.
  '_pai_bpy.ops.export_scene.gltf(filepath=_pai_out, export_format="GLB", export_cameras=True, export_animations=True)',
