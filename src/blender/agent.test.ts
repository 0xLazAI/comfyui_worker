...
// Several tests pass '/tmp/blender-job' as the codex working directory. buildThreadOptions now
// validates that the directory exists (codex `--cd` fails with "os error 2" otherwise), so ensure
// the placeholder directory is present on disk before those tests run.
beforeAll(() => {
  mkdirSync('/tmp/blender-job', { recursive: true });
});
...
test('assertWorkingDirectoryExists accepts an existing directory', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'blender-cwd-ok-'));
  try {
    expect(() => assertWorkingDirectoryExists(dir)).not.toThrow();
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test('assertWorkingDirectoryExists rejects a missing directory with an actionable message', () => {
  const missing = path.join(os.tmpdir(), 'blender-cwd-does-not-exist-xyz');
  expect(() => assertWorkingDirectoryExists(missing)).toThrow(/does not exist/);
  expect(() => assertWorkingDirectoryExists(missing)).toThrow(/os error 2/);
});

test('assertWorkingDirectoryExists rejects an empty path', () => {
  expect(() => assertWorkingDirectoryExists('   ')).toThrow(/empty/);
});

test('assertWorkingDirectoryExists rejects a path that is a file, not a directory', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'blender-cwd-file-'));
  const filePath = path.join(dir, 'model.glb');
  await writeFile(filePath, 'not a dir');
  try {
    expect(() => assertWorkingDirectoryExists(filePath)).toThrow(/not a directory/);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
