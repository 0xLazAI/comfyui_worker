...
// The three-view → 3D (Hunyuan3D-mv) modeling backend is the SAME host as Stephen
// render, reached over its /api/modeling HTTP surface (upload views → submit → poll
// → download GLB). So it reuses STEPHEN_RENDER_BASE_URL directly — one base URL, no
// separate PAILANG_STUDIO_BASE_URL env var.
